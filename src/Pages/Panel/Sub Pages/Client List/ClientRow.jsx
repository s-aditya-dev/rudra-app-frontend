...
import { getCompanyName } from "./utils.jsx";
...
const ClientRow = ({
  ...
  const lastRemarkID =
    lastVisit.visitRemarkId[lastVisit.visitRemarkId.length - 1];
  let lastRemark = "No remark available for this record";
  if (lastRemarkID && lastRemarkID.visitRemark) {
    lastRemark = lastRemarkID.visitRemark;
  }
  ...
  return (
    ...
    >
      ...
      <td
        data-cell="Requirement"
        onClick={() => handleConditionalClick(client._id)}
      >
        {client.requirement}
      </td>
      <td data-cell="Budget" onClick={() => handleConditionalClick(client._id)}>
        {client.budget ? formatBudget(client.budget) : ""}
      </td>
      {lastVisit && (
        <>
          <td
            ...
            onClick={() => handleConditionalClick(client._id)}
          >
            {getCompanyName(lastVisit.referenceBy, companies)}
          </td>
          <td
            data-cell="Source"
            onClick={() => handleConditionalClick(client._id)}
          ...
          <td
            data-cell="Status"
            onClick={() => onRowClick(client._id)}
            className={getStatusClass(lastVisit.status)}
          >
            {lastVisit.status}
          </td>
        </>
      ...
      <td data-cell="Remark">{lastRemark}</td>
    </tr>
  );
};
